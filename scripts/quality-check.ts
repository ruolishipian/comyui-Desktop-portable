...
const checkModules = (): void => {
  ...
  modules.forEach((module: string) => {
    const modulePath: string = path.join(projectRoot, 'src', 'modules', module);
...
// 2. 检查核心文件
const checkCoreFiles = (): void => {
  const coreFiles: Array<{ path: string; name: string }> = [
  ...
  coreFiles.forEach((item: { path: string; name: string }) => {
    const filePath: string = path.join(projectRoot, item.path);
...
const checkAssets = (): void => {
  const assets: string[] = ['loading.html', 'error.html', 'select-env.html', 'log.html', 'settings.html'];

  assets.forEach((asset: string) => {
    const assetPath: string = path.join(projectRoot, 'assets', asset);
...
// 4. 检查模块导出一致性
const checkModuleExports = (): void => {
  const modulePath: string = path.join(projectRoot, 'src', 'modules', 'index.ts');
  if (!fs.existsSync(modulePath)) {
    results.fail.push('模块索引文件不存在');
    return;
  }

  const content: string = fs.readFileSync(modulePath, 'utf8');
  ...
  requiredExports.forEach((exp: string) => {
...
const checkDependencies = (): void => {
  const packagePath: string = path.join(projectRoot, 'package.json');
  ...
  try {
    const content = JSON.parse(fs.readFileSync(packagePath, 'utf8')) as {
      dependencies?: Record<string, string>;
      devDependencies?: Record<string, string>;
    };
    const requiredDeps: string[] = ['electron', 'electron-store', 'tree-kill', 'typescript'];
...
// 6. 检查模块引用
const checkModuleImports = (): void => {
  const mainPath: string = path.join(projectRoot, 'src', 'main.ts');
  if (!fs.existsSync(mainPath)) {
    results.fail.push('main.ts 不存在');
    return;
  }

  const content: string = fs.readFileSync(mainPath, 'utf8');
...
// 7. 检查类型定义文件
const checkTypeDefinitions = (): void => {
  const typeFiles: string[] = ['src/types/index.d.ts', 'src/types/electron.d.ts'];

  typeFiles.forEach((typeFile: string) => {
    const typePath: string = path.join(projectRoot, typeFile);
...
// 9. 检查 IPC 通道一致性
const checkIpcChannelsConsistency = (): void => {
  const ipcChannelsPath: string = path.join(projectRoot, 'src', 'constants', 'ipc-channels.ts');
  const preloadPath: string = path.join(projectRoot, 'src', 'preload.ts');
  ...
  const ipcChannelsFileContent: string = fs.readFileSync(ipcChannelsPath, 'utf8');
  const preloadFileContent: string = fs.readFileSync(preloadPath, 'utf8');
  ...
  // 提取键值对
  const extractChannels = (content: string): Record<string, string> => {
...
// 10. 检查依赖类型定义
const checkDependencyTypes = (): void => {
  // 检查 tree-kill 自带类型
  const treeKillTypesPath: string = path.join(projectRoot, 'node_modules', 'tree-kill', 'index.d.ts');
  ...
  // 检查 @types/node
  const typesNodePath: string = path.join(projectRoot, 'node_modules', '@types', 'node');
...
const runAllChecks = (): void => {
  ...
  // 输出结果
  console.log('✅ 通过项：');
  results.pass.forEach((item: string) => console.log(`  - ${item}`));

  console.log('\n⚠️  警告项：');
  if (results.warn.length === 0) {
    console.log('  无');
  } else {
    results.warn.forEach((item: string) => console.log(`  - ${item}`));
  }
  ...
  if (results.fail.length === 0) {
    ...
    results.fail.forEach((item: string) => console.log(`  - ${item}`));
  }
